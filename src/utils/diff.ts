...
export function highlightAdditions(before: string, after: string): string {
  ...
  if (after.includes(before) && before !== after) {
    ...
    return `${before} ${addition.split(' ').map(word => `\x1b[32m${word}\x1b[0m`).join(' ')}`;
  }
 ...
 */
export function formatMessageDiff(before: string, after: string, type: 'add' | 'remove'): string {
  if (type === 'add') {
