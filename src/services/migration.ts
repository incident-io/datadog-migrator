...
export class MigrationService {
  ...
  private getWebhookNameForTeam(team?: string): string {
    ...
    const webhookName = this.incidentioConfig.webhookNameFormat.replace('{team}', team);
  ...
  private getTeamForPagerDutyService(service: string): string | undefined {
    ...
    debug(`Looking up team for PagerDuty service "${service}": ${mapping?.incidentioTeam ?? 'not found'}`);
   ...
   */
  private filterMonitors(monitors: DatadogMonitor[], filterOptions?: FilterOptions): DatadogMonitor[] {
    ...
    return monitors.filter(monitor => {
      ...
      if (filterOptions.namePattern && !filterOptions.namePattern.test(monitor.name)) {
        return false;
      }
      
      // Filter by message pattern
      if (filterOptions.messagePattern && !filterOptions.messagePattern.test(monitor.message)) {
        return false;
      }
  ...
  async migrateMonitors(options: MigrationOptions): Promise<{ 
    ...
    const monitors = options.filter ? this.filterMonitors(allMonitors, options.filter) : allMonitors;
    
    // Validate mappings if requested
    let validationResults;
    if (options.validateMappings && options.type === MigrationType.ADD_INCIDENTIO_WEBHOOK) {
      ...
      if (!validationResults.valid && !options.dryRun) {
        if (validationResults.unmappedServices.length > 0) {
          throw new Error(`Missing mappings for PagerDuty services: ${validationResults.unmappedServices.join(', ')}`);
        }
    ...
    let updated = 0;
    let unchanged = 0;
    const changes: { id: number, name: string, before: string, after: string, reason?: string }[] = [];
  ...
  async processMonitor(monitor: DatadogMonitor, options: MigrationOptions): Promise<{ 
    ...
    switch (options.type) {
      case MigrationType.ADD_INCIDENTIO_WEBHOOK:
        ...
        debug(`Found PagerDuty services in monitor ${monitor.id}: ${JSON.stringify(pagerDutyServices)}`);
        
        if (pagerDutyServices.length === 0) {
          debug(`No PagerDuty services found in monitor ${monitor.id}`);
          return { updated: false, message: newMessage, reason: 'No PagerDuty services found' };
        }

        // Check for existing incident.io webhooks
        const existingWebhooks = this.findIncidentioWebhooks(message);
        debug(`Found existing webhooks in monitor ${monitor.id}: ${JSON.stringify(existingWebhooks)}`);
        ...
        if (options.singleWebhook) {
          ...
          if (existingWebhooks.length > 0) {
            if (!existingWebhooks.some(webhook => webhook === `@${webhookName}`)) {
              ...
              return { updated: false, message: newMessage, reason: 'Already has correct incident.io webhook' };
            }
          }
          
          // No existing webhook, add the new one
          debug(`Adding default webhook @${webhookName} to monitor ${monitor.id}`);
          ...
          debug(`Expected webhooks for monitor ${monitor.id}: ${[...expectedWebhooks].join(', ')}`)
          ...
          const needsUpdate = 
            ...
            ![...expectedWebhooks].every(webhook => existingWebhooksSet.has(webhook)); // Different webhooks
          ...
          if (needsUpdate) {
            ...
            debug(`Monitor ${monitor.id} already has all correct incident.io webhooks`);
            return { updated: false, message: newMessage, reason: 'Already has all correct incident.io webhooks' };
          }
        }
        break;
      ...
      case MigrationType.REMOVE_INCIDENTIO_WEBHOOK:
        ...
        if (incidentWebhooks.length === 0) {
          return { updated: false, message: newMessage, reason: 'No incident.io webhooks found' };
        }
    ...
    if (updated && !dryRun) {
      debug(`EXECUTING UPDATE - Attempting to update monitor ${monitor.id} with message: "${newMessage}"`);
      try {
        const updatedMonitor = await this.datadogService.updateMonitor(monitor.id, { message: newMessage });
        debug(`Successfully updated monitor ${monitor.id} - Response ID: ${updatedMonitor.id}`);
        console.log(`Successfully updated monitor ${monitor.id} (${monitor.name})`);
      } catch (error) {
        debug(`CRITICAL ERROR - Failed to update monitor ${monitor.id}: ${error instanceof Error ? error.message : String(error)}`);
        console.error(`Failed to update monitor ${monitor.id}: ${error instanceof Error ? error.message : String(error)}`);
