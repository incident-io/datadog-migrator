...
Deno.test("analyze command - with PagerDuty provider", async () => {
  await withMockedEnv(async () => {
    ...
    assertEquals(
      incidentioOnly.length,
      1,
      "incident.io-only count should be 1",
    );
    ...
    assertEquals(
      unmappedServices.length,
      0,
      "All services should have mappings",
    );
    ...
    assertEquals(
      nullMappings.length,
      0,
      "All mappings should have teams assigned",
    );
  });
});
...
Deno.test("analyze command - with Opsgenie provider and unmapped services", async () => {
  await withMockedEnv(async () => {
    ...
    assertEquals(
      unmappedServices[0],
      "new-service",
      "Unmapped service should be 'new-service'",
    );
  });
});
...
Deno.test("analyze command - with filters", async () => {
  await withMockedEnv(async () => {
    ...
    assertEquals(
      prodMonitors.length,
      3,
      "Should have 3 monitors with env:prod tag",
    );

    // Filter by tag "service:api"
    const apiMonitors = allMonitors.filter((m) =>
      m.tags.includes("service:api")
    );
    assertEquals(
      apiMonitors.length,
      2,
      "Should have 2 monitors with service:api tag",
    );
    ...
    assertEquals(
      nameFilteredMonitors.length,
      3,
      "Should have 3 monitors with 'API' in the name",
    );
    ...
    assertEquals(
      messageFilteredMonitors.length,
      2,
      "Should have 2 monitors with 'pagerduty-api' in the message",
    );
  });
});
