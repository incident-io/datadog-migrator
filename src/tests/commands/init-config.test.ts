...
import { createOpsgenieMonitor, createPagerDutyMonitor } from "../utils/test-utils.ts";
...
Deno.test("init-config - default config creation", async () => {
  ...
  await withMockedEnv(async () => {
    ...
    assertEquals(
      defaultConfig.incidentioConfig.source,
      "pagerduty",
      "Default provider should be PagerDuty",
    );
    assertEquals(
      defaultConfig.incidentioConfig.webhookPerTeam,
      false,
      "Default should use single webhook mode",
    );
    assertEquals(
      defaultConfig.incidentioConfig.addTeamTags,
      false,
      "Default should not add team tags",
    );
    assertEquals(
      defaultConfig.incidentioConfig.teamTagPrefix,
      "team",
      "Default team tag prefix should be 'team'",
    );
    assertEquals(
      defaultConfig.mappings.length,
      0,
      "Default should have no mappings",
    );
  });
});
...
Deno.test("init-config - PagerDuty service detection", async () => {
  await withMockedEnv(async () => {
    ...
    assertEquals(
      ...
      3,
      "Should detect 3 PagerDuty services",
    );
    assertEquals(
      detectedServices[0],
      "api-critical",
      "Should detect api-critical service",
    );
    assertEquals(
      detectedServices[1],
      "api-non-critical",
      "Should detect api-non-critical service",
    );
    assertEquals(
      detectedServices[2],
      "database",
      "Should detect database service",
    );
    ...
    assertEquals(
      firstMapping.pagerdutyService,
      "api-critical",
      "First mapping should be for api-critical",
    );
    assertEquals(
      firstMapping.incidentioTeam,
      null,
      "Team should be null initially",
    );
    assertEquals(
    ...
    assertEquals(
      secondMapping.additionalMetadata,
      undefined,
      "Other mappings shouldn't have additionalMetadata by default",
    );
  });
});
...
Deno.test("init-config - Opsgenie service detection", async () => {
  await withMockedEnv(async () => {
    ...
    assertEquals(
      detectedServices.length,
      3,
      "Should detect 3 Opsgenie services",
    );
    assertEquals(
      detectedServices[0],
      "api-critical",
...
Deno.test("init-config - team webhook configuration", async () => {
  ...
  await withMockedEnv(async () => {
    ...
    assertEquals(
      config.incidentioConfig.webhookPerTeam,
      true,
      "Should enable team-specific webhooks",
    );
    assertEquals(
      config.incidentioConfig.addTeamTags,
      false,
      "Should not add team tags with team webhooks",
    );

    // Verify webhook token handling
    assertEquals(
      config.incidentioConfig.webhookToken,
      "test-token",
      "Should have a webhook token",
    );
  });
});
...
Deno.test("init-config - single webhook with team tags configuration", async () => {
  ...
  await withMockedEnv(async () => {
    ...
    assertEquals(
      config.incidentioConfig.webhookPerTeam,
      false,
      "Should use single webhook",
    );
    assertEquals(
      config.incidentioConfig.addTeamTags,
      true,
      "Should add team tags",
    );
    assertEquals(
      config.incidentioConfig.teamTagPrefix,
      "squad",
      "Should use the specified tag prefix",
    );
  });
});
