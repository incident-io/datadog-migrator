...
Deno.test("additionalMetadata with team-specific webhooks", async () => {
  await withMockedEnv(async () => {
    ...
    console.log("Webhook calls made:", JSON.stringify(mockService.webhookCalls));
    
    // Verify the update calls
    assertEquals(mockService.updateCalls.length, 1, "Should make one update call");
    
    // Verify the webhook calls
    assertEquals(mockService.webhookCalls.length, 1, "Should create one webhook");
    
    // Check that the webhook payload includes the additional metadata
    const webhook = mockService.webhookCalls[0];
    assertEquals(webhook.name, "incident-io-api-team", "Webhook should have team name");
    
    // Verify the payload contains our metadata
    assertStringIncludes(webhook.payload, '"priority": "high"', "Webhook payload should include priority metadata");
    assertStringIncludes(webhook.payload, '"service": "api"', "Webhook payload should include service metadata");
    ...
    assertStringIncludes(updateCall.data.message as string, "@webhook-incident-io-api-team", 
