...
export async function testAdditionalMetadata(
  ...
  await withMockedEnv(async () => {
    ...
    // Verify update calls
    assertEquals(
      mockService.apiCalls.updateMonitor.length,
      1,
      "Expected exactly one monitor update call",
    );

    // Verify webhook call if a team-specific webhook is expected
    if (webhookName) {
      assertEquals(
        mockService.apiCalls.createWebhook.length,
        1,
        "Expected exactly one webhook creation call",
      );
      assertEquals(
        mockService.apiCalls.createWebhook[0].name,
        webhookName,
        "Webhook name doesn't match expected value",
      );
      ...
      if (assertWebhookPayloadIncludes) {
        for (const expectedText of assertWebhookPayloadIncludes) {
          assertEquals(
            mockService.apiCalls.createWebhook[0].payload.includes(
              expectedText,
            ),
            true,
