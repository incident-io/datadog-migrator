...
import { DatadogConfig, IncidentioConfig, DatadogMonitor } from '../types';
  ...
  program
    ...
    .action(async (options) => {
      ...
      try {
        ...
        const stats = analyzeMonitors(filteredMonitors, mappings);
        ...
        if (options.showMonitors) {
          displayMonitorDetails(filteredMonitors, stats);
        }
      } catch (error) {
 ...
 */
function filterMonitors(monitors: DatadogMonitor[], options: any): DatadogMonitor[] {
 ...
 */
function analyzeMonitors(monitors: DatadogMonitor[], mappings: any[]): MonitorStats {
  ...
  for (const monitor of monitors) {
    ...
    if (hasPagerDuty) {
      ...
      for (const match of pdMatches) {
        ...
        stats.pagerduty.services[service] = (stats.pagerduty.services[service] || 0) + 1;
      }
    }
    ...
    if (hasIncidentio) {
      ...
      for (const match of incidentMatches) {
        ...
        stats.incidentio.webhooks[webhook] = (stats.incidentio.webhooks[webhook] || 0) + 1;
      }
    }
...
function displayStats(stats: MonitorStats, totalFiltered: number): void {
  ...
  if (Object.keys(stats.pagerduty.services).length > 0) {
    ...
    const sortedServices = Object.entries(stats.pagerduty.services)
      .sort((a, b) => b[1] - a[1]); // Sort by count, descending
    
    for (const [service, count] of sortedServices) {
      console.log(`  ${kleur.blue(service)}: ${count} monitors (${getPercentage(count)})`);
    }
  }
  ...
  if (Object.keys(stats.incidentio.webhooks).length > 0) {
    ...
    const sortedWebhooks = Object.entries(stats.incidentio.webhooks)
      .sort((a, b) => b[1] - a[1]); // Sort by count, descending
    
    for (const [webhook, count] of sortedWebhooks) {
      console.log(`  ${kleur.green(webhook)}: ${count} monitors (${getPercentage(count)})`);
    }
  }
}
 ...
 */
function validateMappings(stats: MonitorStats, mappings: any[]): void {
  ...
  const nullMappings = services.filter(s => 
    mappingsMap.has(s) && mappingsMap.get(s) === null
  );
  
  console.log(kleur.bold('\nMapping Validation:'));
  
  if (unmappedServices.length === 0 && nullMappings.length === 0) {
    console.log(kleur.green('  ✓ All PagerDuty services have complete mappings'));
  } else {
    // Show unmapped services
    if (unmappedServices.length > 0) {
      console.log(kleur.red(`  ✗ ${unmappedServices.length} PagerDuty services lack mappings:`));
    ...
    if (nullMappings.length > 0) {
      console.log(kleur.yellow(`  ! ${nullMappings.length} PagerDuty services have incomplete mappings (null team):`));
      nullMappings.forEach(service => {
        const count = stats.pagerduty.services[service];
        console.log(kleur.yellow(`    - ${service} (used in ${count} monitors)`));
      });
    }
 ...
 */
function displayMonitorDetails(monitors: DatadogMonitor[], stats: MonitorStats): void {
  console.log(kleur.bold('\nMonitor Details:'));
  
  // Group monitors by their notification configuration
  const pdOnly = monitors.filter(monitor => {
    return monitor.message.match(/@pagerduty-(\S+)/g) && !monitor.message.match(/@webhook-incident-io(-\S+)?/g);
  });
...
function extractMentions(message: string): string {
  ...
  result = result.replace(/@pagerduty-(\S+)/g, match => `${kleur.blue(match)}`);
  
  // Highlight incident.io webhooks
  result = result.replace(/@webhook-incident-io(-\S+)?/g, match => `${kleur.green(match)}`);
